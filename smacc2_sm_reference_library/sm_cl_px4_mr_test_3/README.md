...
Test state machine for the `cl_px4_mr` PX4 multirotor client library in Gazebo. Demonstrates the **continuous pattern behaviors** with timer-driven state exits: wait for readiness, arm, takeoff to 5m, cruise along the entry heading with sinusoidal altitude for 30 s (`CbSineAltitudeCruise`, ±1.5 m at 20 m wavelength — 3 sine cycles), hold position while sweeping the heading sinusoidally for 30 s (`CbYawScan`, ±0.6 rad at 6 s period — 5 sweep cycles), return to base, and land. Both pattern behaviors are continuous — they never post success; a 30 s `CbTimerCountdownOnce` in the second orthogonal (`EvTimer`) drives each transition, and each behavior restores its base reference (entry altitude / base heading) on exit.
...
```
colcon build --packages-select cl_px4_mr sm_cl_px4_mr_test_3
```
...
ros2 launch sm_cl_px4_mr_test_3 sm_cl_px4_mr_test_3.launch.py
```
...
| Component | Location |
|-----------|----------|
| State Machine (ROS 2) | `~/.ros/log/` (latest node log: `sm_cl_px4_mr_test_3_node_*.log`) |
