...
#include <sm_dance_bot_warehouse/action/led_control.hpp>
#include <std_msgs/msg/u_int8.hpp>
#include <thread>
// #include <sm_dance_bot_warehouse/action/LEDControlActionResult.h>
// #include <sm_dance_bot_warehouse/action/LEDControlResult.h>
...
public:
  ...
  using GoalHandleLEDControl =
    rclcpp_action::ServerGoalHandle<sm_dance_bot_warehouse::action::LEDControl>;
...
*/
  ...
  {
    currentState_ = sm_dance_bot_warehouse::action::LEDControl::Result::STATE_UNKNOWN;
  }
...
*/
                 ...
                 gh)  // Note: "Action" is not appended to DoDishes here
  {
    ...
    if (goal->command == sm_dance_bot_warehouse::action::LEDControl_Goal::CMD_ON)
    {
      RCLCPP_INFO(this->get_logger(), "ON");
      currentState_ = sm_dance_bot_warehouse::action::LEDControl_Result::STATE_RUNNING;
    }
    else if (goal->command == sm_dance_bot_warehouse::action::LEDControl_Goal::CMD_OFF)
    {
      RCLCPP_INFO(this->get_logger(), "OFF");
      currentState_ = sm_dance_bot_warehouse::action::LEDControl_Result::STATE_IDLE;
    }

    auto feedback_msg = std::make_shared<sm_dance_bot_warehouse::action::LEDControl::Feedback>();
    ...
    auto result = std::make_shared<sm_dance_bot_warehouse::action::LEDControl::Result>();
  ...
  rclcpp_action::GoalResponse handle_goal(
    ...
    std::shared_ptr<const sm_dance_bot_warehouse::action::LEDControl::Goal> /*goal*/)
  {
    // (void)uuid;
...
*/
  void publishStateMarker()
  {
    ...
    if (currentState_ == sm_dance_bot_warehouse::action::LEDControl::Result::STATE_RUNNING)
    {
      // show green ball
      marker.color.r = 0;
      marker.color.g = 1;
    ...
    else if (currentState_ == sm_dance_bot_warehouse::action::LEDControl::Result::STATE_IDLE)
    {
      // show gray ball
      marker.color.r = 0.7;
      marker.color.g = 0.7;
