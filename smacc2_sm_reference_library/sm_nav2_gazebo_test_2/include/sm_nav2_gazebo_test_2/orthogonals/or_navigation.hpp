...
public:
  void onInitialize() override
  {
    ...
    // CpOdomTracker: records the forward path that CbUndoPathBackwards retraces
    client->createComponent<cl_nav2z::odom_tracker::CpOdomTracker>();

    // CpPlannerSwitcher: switches between planners/controllers (planner_selector topics)
    client->createComponent<cl_nav2z::CpPlannerSwitcher>();

    // CpGoalCheckerSwitcher: switches goal checker algorithms (goal_checker_selector topic)
