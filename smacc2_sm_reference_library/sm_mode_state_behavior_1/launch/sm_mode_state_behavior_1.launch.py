...
def setup_log_directory():
    ...
    try:
        ...
        fallback_dir = os.path.join("/tmp", "sm_mode_state_behavior_1_logs", timestamp)
        try:
...
def generate_launch_description():
    ...
    if log_dir:
        ...
        sm_prefix = f"konsole --hold -p tabtitle='SM Mode State Behavior 1' -e bash -c 'RCUTILS_COLORIZED_OUTPUT=1 \"$@\" 2>&1 | tee {sm_log}; exec bash' -- "
        ...
        sm_prefix = "konsole --hold -p tabtitle='SM Mode State Behavior 1' -e"
        kb_prefix = "konsole --hold -p tabtitle='Keyboard Server' -e"

    return LaunchDescription(
        [
            Node(
                package="sm_mode_state_behavior_1",
                executable="sm_mode_state_behavior_1_node",
                name="sm_mode_state_behavior_1",
                output="screen",
                prefix=sm_prefix,
