...
// INNER STATE: fly the pattern at the pin, then on to StTransitToVSChain3
struct StiVSChain2Run : smacc2::SmaccState<StiVSChain2Run, SsVSChain2>
{
  using SmaccState::SmaccState;

  typedef mpl::list<
    Transition<EvCbSuccess<CbVSSearch, OrPx4>, StTransitToVSChain3, SUCCESS>,
  ...
  {
    const auto p = SsVSChain2::patternParams();
    auto * cb = this->getClientBehavior<OrPx4, CbVSSearch>();
    cb->setParams(p);
    cb->setFollowerParams(SsVSChain2::followerParams());
    cb->setTimeout(patternTimeout(flightPatternVSSearchLength(p), kPatternSpeedMps));

    const NedXY pin = SsVSChain2::pin();
    RCLCPP_INFO(
      getLogger(), "StiVSChain2Run: pin P10 NED (%.1f, %.1f)", static_cast<double>(pin.x),
