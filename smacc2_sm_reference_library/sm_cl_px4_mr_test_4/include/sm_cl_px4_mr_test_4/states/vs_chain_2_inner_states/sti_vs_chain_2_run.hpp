...
// INNER STATE: run pearl #2 of the sector-search chain, then return to StRailway
struct StiVSChain2Run : smacc2::SmaccState<StiVSChain2Run, SsVSChain2>
{
  ...
  void runtimeConfigure()
  {
    ...
    const auto & params = this->context<SsVSChain2>().params;
    ...
    RCLCPP_INFO(
      getLogger(), "StiVSChain2Run: pin '%s' NED (%.1f, %.1f)", pin.name.c_str(),
