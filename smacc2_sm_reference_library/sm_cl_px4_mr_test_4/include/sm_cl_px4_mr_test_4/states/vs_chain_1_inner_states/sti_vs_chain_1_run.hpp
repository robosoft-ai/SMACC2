...
// INNER STATE: fly the pattern at the pin, then on to StTransitToVSChain2
struct StiVSChain1Run : smacc2::SmaccState<StiVSChain1Run, SsVSChain1>
{
  using SmaccState::SmaccState;

  typedef mpl::list<
    Transition<EvCbSuccess<CbVSSearch, OrPx4>, StTransitToVSChain2, SUCCESS>,
  ...
  {
    const auto p = SsVSChain1::patternParams();
    auto * cb = this->getClientBehavior<OrPx4, CbVSSearch>();
    cb->setParams(p);
    cb->setFollowerParams(SsVSChain1::followerParams());
    cb->setTimeout(patternTimeout(flightPatternVSSearchLength(p), kPatternSpeedMps));

    const NedXY pin = SsVSChain1::pin();
    RCLCPP_INFO(
      getLogger(), "StiVSChain1Run: pin P9 NED (%.1f, %.1f)", static_cast<double>(pin.x),
