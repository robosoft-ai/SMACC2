...
// INNER STATE: run pearl #1 of the sector-search chain, then return to StRailway
struct StiVSChain1Run : smacc2::SmaccState<StiVSChain1Run, SsVSChain1>
{
  ...
  void runtimeConfigure()
  {
    ...
    const auto & params = this->context<SsVSChain1>().params;
    ...
    RCLCPP_INFO(
      getLogger(), "StiVSChain1Run: pin '%s' NED (%.1f, %.1f)", pin.name.c_str(),
