...
// TRANSIT STATE: vertical sine-wave leg from lawnmower 2 to the entry of the grid (P5)
struct StTransitToGridPattern1 : smacc2::SmaccState<StTransitToGridPattern1, MsInFlight>
{
  using SmaccState::SmaccState;

  typedef mpl::list<
    Transition<EvCbSuccess<CbSineWaveVertical, OrPx4>, SsGridPattern1, SUCCESS>,
  ...
  {
    const NedXY from = SsLawnmower2::exit();
    const NedXY to = SsGridPattern1::entry();
    ...
    RCLCPP_INFO(
      getLogger(), "StTransitToGridPattern1: -> P5 entry NED (%.1f, %.1f), %.0f m", static_cast<double>(to.x),
