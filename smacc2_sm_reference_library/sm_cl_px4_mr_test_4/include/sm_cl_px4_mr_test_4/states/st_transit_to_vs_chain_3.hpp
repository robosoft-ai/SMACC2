...
// TRANSIT STATE: vertical sine-wave leg from pearl 2 to pearl 3 (P11)
struct StTransitToVSChain3 : smacc2::SmaccState<StTransitToVSChain3, MsInFlight>
{
  using SmaccState::SmaccState;

  typedef mpl::list<
    Transition<EvCbSuccess<CbSineWaveVertical, OrPx4>, SsVSChain3, SUCCESS>,
  ...
  {
    const NedXY from = SsVSChain2::exit();
    const NedXY to = SsVSChain3::entry();
    ...
    RCLCPP_INFO(
      getLogger(), "StTransitToVSChain3: -> P11 entry NED (%.1f, %.1f), %.0f m", static_cast<double>(to.x),
