...
// TRANSIT STATE: horizontal sine-wave leg from square spiral 1 to the entry of square spiral 2 (P2)
struct StTransitToSquareSpiral2 : smacc2::SmaccState<StTransitToSquareSpiral2, MsInFlight>
{
  using SmaccState::SmaccState;

  typedef mpl::list<
    Transition<EvCbSuccess<CbSineWaveHorizontal, OrPx4>, SsSquareSpiral2, SUCCESS>,
  ...
  {
    const NedXY from = SsSquareSpiral1::exit();
    const NedXY to = SsSquareSpiral2::entry();
    ...
    RCLCPP_INFO(
      getLogger(), "StTransitToSquareSpiral2: -> P2 entry NED (%.1f, %.1f), %.0f m", static_cast<double>(to.x),
