...
// INNER STATE: fly the pattern at the pin, then on to SsVSSearch3
struct StiVSSearch2Run : smacc2::SmaccState<StiVSSearch2Run, SsVSSearch2>
{
  using SmaccState::SmaccState;

  typedef mpl::list<
    Transition<EvCbSuccess<CbVSSearch, OrPx4>, SsVSSearch3, SUCCESS>,
  ...
  {
    const auto p = SsVSSearch2::patternParams();
    auto * cb = this->getClientBehavior<OrPx4, CbVSSearch>();
    cb->setParams(p);
    cb->setFollowerParams(SsVSSearch2::followerParams());
    cb->setTimeout(patternTimeout(flightPatternVSSearchLength(p), kPatternSpeedMps));

    const NedXY pin = SsVSSearch2::pin();
    RCLCPP_INFO(
      getLogger(), "StiVSSearch2Run: pin P7 NED (%.1f, %.1f)", static_cast<double>(pin.x),
