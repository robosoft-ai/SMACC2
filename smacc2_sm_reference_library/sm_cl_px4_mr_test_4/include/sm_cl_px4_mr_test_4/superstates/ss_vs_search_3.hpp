...
// SUPERSTATE: Victor Sierra sector search #3, rotated 60 deg, same datum as #1.
...
// behavior.
struct SsVSSearch3 : smacc2::SmaccState<SsVSSearch3, MsInFlight, StiVSSearch3Run>
{
  using SmaccState::SmaccState;

  // P8: ring station 5, same datum as SsVSSearch1
  ...
  static cl_px4_mr::FlightPatternVSSearchParams patternParams()
  {
    ...
    p.initialHeading = kVSSearchInitialHeading3;
  ...
  void onEntry()
  {
    ...
    RCLCPP_INFO(
      getLogger(), "=== SsVSSearch3: sector search, r=%.0f m, first leg %.0f deg, ~%.0f m, pin P8 ===",
      p.radius, p.initialHeading * 180.0 / M_PI, cl_px4_mr::flightPatternVSSearchLength(p));
  }

  void onExit()
  {
    RCLCPP_INFO(getLogger(), "=== Exiting SsVSSearch3 ===");
  }
};
