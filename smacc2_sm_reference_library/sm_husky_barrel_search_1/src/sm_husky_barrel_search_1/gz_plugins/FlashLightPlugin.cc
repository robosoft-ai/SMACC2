...
namespace smacc2
{
  ...
  class FlashLightSettingPrivate
  {
    ...
    public: gazebo::physics::LinkPtr FindLinkForLight(
    ...
    public: gazebo::transport::PublisherPtr pubLight;
    ...
    while (sdfBlock)
    {
      ...
      else
      {
        //gzerr << "Parameter <duration> is missing in a block." << std::endl;
             block->duration = std::numeric_limits<double>::max();
      ...
      else
      {
        //gzerr << "Parameter <interval> is missing in a block." << std::endl;
                   block->interval = std::numeric_limits<double>::max();
...
void FlashLightSetting::InitPubLight(
  const gazebo::transport::PublisherPtr &_pubLight)
{
  // The PublisherPtr
...
//////////////////////////////////////////////////
void FlashLightSetting::UpdateLightInEnv(const gazebo::common::Time &_currentTime)
{
...
//////////////////////////////////////////////////
const gazebo::physics::LinkPtr FlashLightSetting::Link() const
{
...
//////////////////////////////////////////////////
FlashLightPlugin::FlashLightPlugin() : gazebo::ModelPlugin(),
...
//////////////////////////////////////////////////
void FlashLightPlugin::Load(gazebo::physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
    ...
    while (sdfFlashLight)
    {
      ...
      if (sdfFlashLight->HasElement("id"))
      {
        ...
        std::shared_ptr<FlashLightSetting> setting
          = this->CreateSetting(
              sdfFlashLight, this->dataPtr->model, currentTime, node);
...
{
  return std::make_shared<FlashLightSetting>(_sdf, _model, _currentTime, node);
}
