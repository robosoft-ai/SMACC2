...
const std::map<std::string, std::shared_ptr<smacc::ISmaccOrthogonal>> &
...
void ISmaccStateMachine::initializeROS(std::string shortname)
{
  ...
  stateMachinePub_ = nh_->create_publisher<smacc_msgs::msg::SmaccStateMachine>(
    shortname + "/smacc/state_machine_description", 1);
  stateMachineStatusPub_ =
    nh_->create_publisher<smacc_msgs::msg::SmaccStatus>(shortname + "/smacc/status", 1);
  transitionLogPub_ = nh_->create_publisher<smacc_msgs::msg::SmaccTransitionLogEntry>(
    shortname + "/smacc/transition_log", 1);

  // STATE MACHINE SERVICES
  transitionHistoryService_ = nh_->create_service<smacc_msgs::srv::SmaccGetTransitionHistory>(
