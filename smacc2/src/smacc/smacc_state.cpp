...
void ISmaccState::param(std::string param_name)
{
...
void ISmaccState::notifyTransitionFromTransitionTypeInfo(TypeInfo::Ptr &transitionType)
{
    ...
    if (currstateinfo != nullptr)
    {
        ...
        RCLCPP_ERROR_STREAM(getNode()->get_logger(),"Transition happened, from current state " << currstateinfo->getDemangledFullName() << " but there is not any transitioninfo match available to publish transition: " << transitionType->getFullName());
        std::stringstream ss;
        ...
        {
            RCLCPP_ERROR_STREAM(getNode()->get_logger()," * Ancestor " << ancestor->getDemangledFullName() << ":");
    ...
    {
        RCLCPP_ERROR_STREAM(getNode()->get_logger(),"Transition happened, but current state was not set. Transition candidates:");
    }
}
