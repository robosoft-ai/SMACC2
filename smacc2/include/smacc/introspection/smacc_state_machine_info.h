...
// smacc_msgs
#include <smacc_msgs/msg/smacc_state.hpp>
#include <smacc_msgs/msg/smacc_transition.hpp>
#include <smacc_msgs/msg/smacc_orthogonal.hpp>
#include <smacc_msgs/msg/smacc_state_reactor.hpp>
...
public:
    ...
    std::vector<smacc_msgs::msg::SmaccState> stateMsgs;
...
{
    RCLCPP_INFO(globalNh_->get_logger(),"State %s Walker transition: %s", sourceState->toShortName().c_str(), demangleSymbol(typeid(Ev).name()).c_str());
...
CallOnDefinition()
{
    ...
    RCLCPP_INFO_STREAM(globalNh_->get_logger(),"EXECUTING ONDEFINITION: " << demangleSymbol(typeid(T).name()));
    T::staticConfigure();
}
...
CallOnDefinition()
{
    RCLCPP_INFO_STREAM(globalNh_->get_logger(), "static OnDefinition: dont exist for " << demangleSymbol(typeid(T).name()));
    /* something when T has toString ... */
}
...
std::shared_ptr<SmaccStateInfo> SmaccStateInfo::createChildState()
{
    ...
    RCLCPP_WARN_STREAM(getNode()->get_logger(),"Real parent state> " << demangleSymbol<typename StateType::TContext>());
