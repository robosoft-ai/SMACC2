...
bool ISmaccOrthogonal::requiresClient(SmaccClientType *&storage)
{
    ...
    RCLCPP_WARN_STREAM(getNode()->get_logger(),"Required client ["<< requiredClientName<< "] not found in current orthogonal. Searching in other orthogonals.");
    ...
    for (auto &orthoentry : this->getStateMachine()->getOrthogonals())
    {
        for (auto &client : orthoentry.second->getClients())
        {
            ...
            if (storage != nullptr)
            {
                RCLCPP_WARN_STREAM(getNode()->get_logger(),"Required client  ["<< requiredClientName<<"] found in other orthogonal.");
                return true;
            }
        }
    }

    RCLCPP_ERROR_STREAM(getNode()->get_logger(),"Required client ["<< requiredClientName<< "] not found even in other orthogonals. Returning null pointer. If the requested client is used may result in a segmentation fault.");
    return false;
}
...
void ISmaccOrthogonal::requiresComponent(SmaccComponentType *&storage)
{
    ...
    {
        RCLCPP_ERROR(getNode()->get_logger(),"Cannot use the requiresComponent funcionality from an orthogonal before onInitialize");
    }
    else
    {
...
public:
    ...
    std::shared_ptr<ClientHandler<TOrthogonal, TClient>> createClient(TArgs... args)
    {
        ...
        RCLCPP_INFO(getNode()->get_logger(),"[%s] creates a client of type '%s' and object tag '%s'",
