...
namespace smacc
{
    ...
    {
        RCLCPP_INFO_STREAM(this->getNode()->get_logger(), "getParam from node: " << getNode()->get_namespace());
...
#define THIS_STATE_NAME ((demangleSymbol(typeid(*this).name()).c_str()))
    ...
    std::shared_ptr<TBehavior> ISmaccState::configure(Args &&... args)
    {
        ...
        RCLCPP_INFO(getNode()->get_logger(),"[%s] Configuring orthogonal: %s", THIS_STATE_NAME, orthogonalkey.c_str());
        ...
        {
            RCLCPP_ERROR(getNode()->get_logger(),"[%s] Skipping client behavior creation in orthogonal [%s]. It does not exist.", THIS_STATE_NAME, orthogonalkey.c_str());
            return nullptr;
        }
    }
    //-------------------------------------------------------------------------------------------------------
    ...
    void ISmaccState::requiresClient(SmaccClientType *&storage)
    {
        ...
        RCLCPP_ERROR(getNode()->get_logger(),"[%s] Client of type '%s' not found in any orthogonal of the current state machine. This may produce a segmentation fault if the returned reference is used.", sname, demangleSymbol<SmaccClientType>().c_str());
    }
    //-------------------------------------------------------------------------------------------------------
