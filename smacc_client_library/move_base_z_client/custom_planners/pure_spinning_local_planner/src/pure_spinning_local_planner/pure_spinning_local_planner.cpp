  ...
  namespace pure_spinning_local_planner
  {
        ...
        const geometry_msgs::msg::Twist &velocity)
    {
      ...
      while (currentPoseIndex_ < (int)plan_.size())
      {
        ...
        if ((currentPoseIndex_ < plan_.size() - 1 && fabs(angular_error) < this->intermediate_goal_yaw_tolerance_) || (currentPoseIndex_ == plan_.size() - 1 && fabs(angular_error) < this->intermediate_goal_yaw_tolerance_))
        {
          currentPoseIndex_++;
        }
        else
      ...
      RCLCPP_DEBUG_STREAM(nh_->get_logger(), "[PureSpinningLocalPlanner] angular error: " << angular_error << "(" << yaw_goal_tolerance_ << ")");
      RCLCPP_DEBUG_STREAM(nh_->get_logger(), "[PureSpinningLocalPlanner] command angular speed: " << cmd_vel.twist.angular.z);
      RCLCPP_DEBUG_STREAM(nh_->get_logger(), "[PureSpinningLocalPlanner] completion " << currentPoseIndex_ << "/" << plan_.size());
