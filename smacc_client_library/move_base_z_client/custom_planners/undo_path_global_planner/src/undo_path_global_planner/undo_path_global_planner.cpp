...
void UndoPathGlobalPlanner::onForwardTrailMsg(const nav_msgs::msg::Path::SharedPtr trailMessage)
{
  ...
  RCLCPP_DEBUG_STREAM(nh_->get_logger(), "[UndoPathGlobalPlanner] received backward path msg poses ["
                                                      ...
                                                      const geometry_msgs::msg::PoseStamped &goal)
{
  ...
  RCLCPP_INFO_STREAM(nh_->get_logger(), " plan checking");
  ...
  RCLCPP_INFO_STREAM(nh_->get_logger(), " plan publishing. size: " << plan.size());
  planPub_->publish(planMsg);
