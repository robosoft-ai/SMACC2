        ...
        void clamp(rclcpp::Node::SharedPtr nh_, geometry_msgs::msg::Twist &cmd_vel, double max_linear_x_speed_, double max_angular_z_speed_)
        {
            ...
            else
            {
                ...
                if (linearAuthority < angularAuthority)
                {
                    ...
                    RCLCPP_INFO_STREAM(nh_->get_logger(), "k=" << kurvature << "lets go to maximum linear capacity: " << cmd_vel);
                }
                else
                {
                    ...
                    RCLCPP_INFO_STREAM(nh_->get_logger(), "lets go to maximum angular capacity: " << cmd_vel);
                }
            }
        }
...
*/
            ...
            const geometry_msgs::msg::Twist &velocity)
        {
            ...
            while (!ok)
            {
                ...
                while (!ok && currentPoseIndex_ < plan_.size())
                {
                    ...
                    if (dist >= carrot_distance_ || angular_error > 0.1)
                    {
                        ...
                        ok = true;
                        RCLCPP_DEBUG(nh_->get_logger(), "current index: %d, carrot goal percentaje: %lf, dist: %lf, maxdist: %lf, angle_error: %lf", currentPoseIndex_, 100.0 * currentPoseIndex_ / plan_.size(), dist, carrot_distance_, angular_error);
                    }
                    else
                    {
                        currentPoseIndex_++;
            ...
            const geometry_msgs::msg::PoseStamped &goalpose = plan_[currentPoseIndex_];
            const geometry_msgs::msg::Point &goalposition = goalpose.pose.position;

            tf2::Quaternion goalQ;
            tf2::fromMsg(goalpose.pose.orientation, goalQ);
            //RCLCPP_INFO_STREAM(nh_->get_logger(),"Plan goal quaternion at "<< goalpose.pose.orientation);

            //goal orientation (global frame)
            double betta = tf2::getYaw(goalpose.pose.orientation) + betta_offset_;
            ...
            RCLCPP_DEBUG_STREAM(nh_->get_logger(), "Forward local planner," << std::endl
            ...
            if (!success)
            {
                RCLCPP_INFO(nh_->get_logger(), "[ForwardLocalPlanner] object detected waiting stopped until it disappears.");
            }
