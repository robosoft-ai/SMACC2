...
{
    RCLCPP_INFO(getNode()->get_logger(),"[CostmapSwitch] registering costmap type: %s", costmapName.c_str());
    auto proxy = std::make_shared<CostmapProxy>(this->moveBaseClient_->name_ + "/" + costmapName, enablePropertyName, getNode());
...
void CostmapSwitch::enable(std::string layerName)
{
    ...
    if (!exists(layerName))
    {
        RCLCPP_ERROR(getNode()->get_logger(),"[CostmapSwitch] costmap %s does not exist", layerName.c_str());
        return;
    }
    else
    {
        RCLCPP_INFO(getNode()->get_logger(),"[CostmapSwitch] costmap %s found. Calling dynamic reconfigure server.", layerName.c_str());
...
CostmapProxy::CostmapProxy(std::string costmap_name, std::string enablePropertyName, rclcpp::Node::SharedPtr nh)
    :nh_(nh)
{
