 ...
 *
 ******************************************************************************************************************/
#include <nitrosz_client/client_behaviors/cb_nitrosz_client_behavior_base.hpp>
#include <nitrosz_client/common.hpp>
namespace cl_nitrosz
{
CbNav2ZClientBehaviorBase::~CbNav2ZClientBehaviorBase() {}

void CbNav2ZClientBehaviorBase::sendGoal(ClNitrosZ::Goal & goal)
{
  RCLCPP_INFO_STREAM(getLogger(), "[" << getName() << "] Creating sending goal callback signal.");

  this->navigationCallback_ = std::make_shared<cl_nitrosz::ClNitrosZ::SmaccNavigateResultSignal>();
  ...
  goalHandleFuture_ = this->nitroszClient_->sendGoal(goal, navigationCallback_);
  RCLCPP_INFO_STREAM(
...
void CbNav2ZClientBehaviorBase::cancelGoal() { this->nitroszClient_->cancelGoal(); }

bool CbNav2ZClientBehaviorBase::isOwnActionResponse(const ClNitrosZ::WrappedResult & r)
{
  auto name = getName();
...
void CbNav2ZClientBehaviorBase::onNavigationResult(const ClNitrosZ::WrappedResult & r)
{
  RCLCPP_DEBUG(
...
void CbNav2ZClientBehaviorBase::onNavigationActionSuccess(const ClNitrosZ::WrappedResult & r)
{
...
void CbNav2ZClientBehaviorBase::onNavigationActionAbort(const ClNitrosZ::WrappedResult & r)
{
  // if (!isOwnActionResponse(r))
  // {
  //   RCLCPP_WARN(
