...
{
using ::cl_nitrosz::odom_tracker::CpOdomTracker;
using ::cl_nitrosz::odom_tracker::WorkingMode;

using ::cl_nitrosz::Pose;
...
void CbNavigateForward::onEntry()
{
  ...
  // get current pose
  auto p = nitroszClient_->getComponent<Pose>();
  ...
  odomTracker_ = nitroszClient_->getComponent<CpOdomTracker>();
  ...
  auto plannerSwitcher = nitroszClient_->getComponent<CpPlannerSwitcher>();
  plannerSwitcher->setForwardPlanner();

  auto goalCheckerSwitcher = nitroszClient_->getComponent<CpGoalCheckerSwitcher>();
