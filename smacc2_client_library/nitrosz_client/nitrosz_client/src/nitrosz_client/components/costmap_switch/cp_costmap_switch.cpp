...
#include <nitrosz_client/components/costmap_switch/cp_costmap_switch.hpp>
  ...
  std::string costmapName, std::string enablePropertyName)
{
  ...
  auto proxy = std::make_shared<CpCostmapProxy>(
    this->nitroszClient_->getName() + "/" + costmapName, enablePropertyName, getNode());
...
{
  this->nitroszClient_ = dynamic_cast<cl_nitrosz::ClNitrosZ *>(owner_);

  if (this->nitroszClient_ == nullptr)
  {
    RCLCPP_ERROR(getLogger(), "the owner of the CpCostmapSwitch must be a ClNitrosZ");
  }
