...
void CpWaypointNavigator::onInitialize() { client_ = dynamic_cast<ClNitrosZ *>(owner_); }

void CpWaypointNavigator::onGoalCancelled(const ClNitrosZ::WrappedResult & /*res*/)
{
  stopWaitingResult();
...
void CpWaypointNavigator::onGoalAborted(const ClNitrosZ::WrappedResult & /*res*/)
{
  stopWaitingResult();
...
void CpWaypointNavigator::onGoalReached(const ClNitrosZ::WrappedResult & /*res*/)
{
...
CpWaypointNavigator::sendNextGoal(
  ...
  cl_nitrosz::ClNitrosZ::SmaccNavigateResultSignal::WeakPtr resultCallback)
{
  if (currentWaypoint_ >= 0 && currentWaypoint_ < (int)waypoints_.size())
  {
    ...
    ClNitrosZ::Goal goal;
    auto p = client_->getComponent<cl_nitrosz::Pose>();
    ...
    auto odomTracker = client_->getComponent<cl_nitrosz::odom_tracker::CpOdomTracker>();
    if (odomTracker != nullptr)
    {
      ...
      odomTracker->setWorkingMode(cl_nitrosz::odom_tracker::WorkingMode::RECORD_PATH);
    }
...
void CpWaypointNavigator::onNavigationResult(const ClNitrosZ::WrappedResult & r)
{
