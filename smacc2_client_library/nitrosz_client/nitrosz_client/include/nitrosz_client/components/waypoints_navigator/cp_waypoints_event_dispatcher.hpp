...
public:
  ...
  void initialize(ClNitrosZ * client);

  void postWaypointEvent(int index);
};

template <typename TEv>
void configurePostEvWaypoint(std::function<void()> * fntarget, ClNitrosZ * client, int index)
{
  fntarget[index] = [=]() { client->template postEvent<TEv>(); };
}

template <typename TDerived, typename TOrthogonal>
void WaypointEventDispatcher::initialize(ClNitrosZ * client)
{
  // Doygen cond tag
  /// @cond
